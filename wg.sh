	...
	while [ "$#" -gt 0 ]; do
		case $1 in
			...
			--cloak)
				...
				autotune=1
				ask_tls_domain=1
				use_tls_domain_as_server_addr=1
				shift
				;;
			-y|--yes)
				assume_yes=1
...
check_args() {
	...
	if [ -n "$tls_domain" ]; then
		if [ -e "$WG_CONF" ]; then
			show_usage "Invalid parameter. --tls-domain can only be specified when installing WireGuard."
		fi
		if ! check_dns_name "$tls_domain"; then
			exiterr "Invalid TLS domain. Must be a fully qualified domain name (FQDN)."
		fi
	fi
...
Install options (optional):
  ...
  --autotune                    adapt MTU and sysctl parameters to this server
  --tls-domain [DNS name]       install a normal HTTPS/TLS site for this domain
  --ask-tls-domain              ask for HTTPS/TLS domain during install
  --cloak                       simple mode: ask for a domain, enable autotune, install HTTPS/TLS site
...
enter_tls_domain() {
	...
	echo
	if [ "$use_tls_domain_as_server_addr" = 1 ]; then
		read -rp "Enter your domain name for WireGuard endpoint and HTTPS/TLS site: " tls_domain_i
		until check_dns_name "$tls_domain_i"; do
			echo "Invalid DNS name. You must enter a fully qualified domain name, e.g. vpn.example.com."
			read -rp "Enter your domain name: " tls_domain_i
		done
		tls_domain="$tls_domain_i"
		server_addr="$tls_domain_i"
		ip="$tls_domain_i"
		show_dns_name_note "$tls_domain_i"
	else
		read -rp "Enter a domain name for the HTTPS/TLS site (press Enter to skip): " tls_domain_i
		while [ -n "$tls_domain_i" ] && ! check_dns_name "$tls_domain_i"; do
			echo "Invalid DNS name. You must enter a fully qualified domain name, e.g. vpn.example.com."
			read -rp "Enter a domain name for the HTTPS/TLS site (press Enter to skip): " tls_domain_i
		done
		if [ -n "$tls_domain_i" ]; then
			tls_domain="$tls_domain_i"
			show_dns_name_note "$tls_domain_i"
		fi
	fi
...
show_config() {
	if [ "$auto" != 0 ]; then
		...
		[ "$autotune" = 1 ] && echo "Auto tune: enabled"
		[ -n "$tls_domain" ] && echo "TLS site: https://$tls_domain"
	fi
}
...
calc_wg_mtu() {
	...
	detect_default_iface
	base_mtu=1500
	if [ -n "$default_iface" ] && [ -r "/sys/class/net/$default_iface/mtu" ]; then
		base_mtu=$(cat "/sys/class/net/$default_iface/mtu")
	fi
	# WireGuard overhead is larger on IPv6 paths. Keep a conservative floor for IPv6 compatibility.
	overhead=60
	[ -n "$ip6" ] && overhead=80
	wg_mtu=$((base_mtu - overhead))
	[ "$wg_mtu" -gt 1420 ] && wg_mtu=1420
	[ "$wg_mtu" -lt 1280 ] && wg_mtu=1280
}

append_adaptive_sysctl() {
	[ "$autotune" != 1 ] && return 0
	...
	backlog=$((cpu_count * 2048))
	[ "$backlog" -lt 4096 ] && backlog=4096
	[ "$backlog" -gt 32768 ] && backlog=32768
cat >> "$conf_opt" <<EOF
# Adaptive WireGuard tuning generated by this installer
net.core.rmem_max = $buf
net.core.wmem_max = $buf
net.core.rmem_default = $((buf / 4))
net.core.wmem_default = $((buf / 4))
...
install_tls_pkgs() {
	[ -z "$tls_domain" ] && return 0
	if [[ "$os" == "ubuntu" || "$os" == "debian" ]]; then
		export DEBIAN_FRONTEND=noninteractive
		(
			set -x
			apt-get -yqq update || apt-get -yqq update
			apt-get -yqq install nginx certbot python3-certbot-nginx >/dev/null
		) || exiterr2
	elif [[ "$os" == "centos" || "$os" == "rhel" ]]; then
		(
			set -x
			yum -y -q install nginx certbot python3-certbot-nginx >/dev/null 2>&1
		) || exiterr3
	elif [[ "$os" == "fedora" ]]; then
		(
			set -x
			dnf install -y nginx certbot python3-certbot-nginx >/dev/null
		) || exiterr "'dnf install' failed."
	elif [[ "$os" == "openSUSE" ]]; then
		(
			set -x
			zypper install -y nginx certbot python3-certbot-nginx >/dev/null
		) || exiterr4
	fi
}
...
open_tls_firewall() {
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		if ! grep -qs -- '--dport 443 -j ACCEPT' /etc/systemd/system/wg-iptables.service 2>/dev/null; then
			sed -i "/ExecStart=.*--dport $port -j ACCEPT/a ExecStart=$iptables_path -w 5 -I INPUT -p tcp --dport 80 -j ACCEPT\nExecStart=$iptables_path -w 5 -I INPUT -p tcp --dport 443 -j ACCEPT" /etc/systemd/system/wg-iptables.service
			sed -i "/ExecStop=.*--dport $port -j ACCEPT/a ExecStop=$iptables_path -w 5 -D INPUT -p tcp --dport 80 -j ACCEPT\nExecStop=$iptables_path -w 5 -D INPUT -p tcp --dport 443 -j ACCEPT" /etc/systemd/system/wg-iptables.service
			systemctl daemon-reload
			systemctl restart wg-iptables.service >/dev/null 2>&1 || true
		fi
	fi
}
...
setup_tls_site() {
	[ -z "$tls_domain" ] && return 0
	install_tls_pkgs
	mkdir -p /var/www/wg-decoy
	cat > /var/www/wg-decoy/index.html <<EOF
<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>$tls_domain</title></head>
<body><h1>$tls_domain</h1><p>It works.</p></body></html>
EOF
	if [ -d /etc/nginx/sites-available ]; then
		nginx_conf=/etc/nginx/sites-available/wg-decoy.conf
		cat > "$nginx_conf" <<EOF
server {
    listen 80;
    server_name $tls_domain;
    root /var/www/wg-decoy;
    index index.html;
    location / { try_files \$uri \$uri/ =404; }
}
EOF
		ln -sf "$nginx_conf" /etc/nginx/sites-enabled/wg-decoy.conf
	else
		nginx_conf=/etc/nginx/conf.d/wg-decoy.conf
		cat > "$nginx_conf" <<EOF
server {
    listen 80;
    server_name $tls_domain;
...
EOF
	...
	echo "$tls_domain" > /etc/wireguard/tls-domain
	open_tls_firewall
	systemctl enable --now nginx >/dev/null 2>&1 || systemctl enable --now nginx.service >/dev/null 2>&1 || true
	nginx -t >/dev/null 2>&1 || exiterr "Nginx configuration test failed."
	systemctl reload nginx >/dev/null 2>&1 || systemctl restart nginx >/dev/null 2>&1
	# Install a real certificate for the supplied domain. The domain must already resolve to this server.
	certbot --nginx -d "$tls_domain" --non-interactive --agree-tos --register-unsafely-without-email --redirect >/dev/null 2>&1 \
		|| echo "Warning: Certbot failed. Check that $tls_domain resolves to this server and ports 80/443 are reachable."
}

remove_tls_site() {
	[ ! -f /etc/wireguard/tls-domain ] && return 0
	domain_to_remove=$(cat /etc/wireguard/tls-domain 2>/dev/null)
	rm -f /etc/nginx/sites-enabled/wg-decoy.conf /etc/nginx/sites-available/wg-decoy.conf /etc/nginx/conf.d/wg-decoy.conf
	rm -rf /var/www/wg-decoy
	rm -f /etc/wireguard/tls-domain
	if [ -n "$domain_to_remove" ] && command -v certbot >/dev/null 2>&1; then
		certbot delete --cert-name "$domain_to_remove" --non-interactive >/dev/null 2>&1 || true
	fi
...
EOF
	...
	client_mtu=$(grep '^MTU' "$WG_CONF" 2>/dev/null | cut -d " " -f 3 | head -1)
	get_export_dir
...
DNS = $dns
PrivateKey = $key
$([ -n "$client_mtu" ] && echo "MTU = $client_mtu")
...
tls_domain=""
ask_tls_domain=0
use_tls_domain_as_server_addr=0
wg_mtu=1420
